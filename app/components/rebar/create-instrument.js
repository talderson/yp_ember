...
export default class CreatePackInstrument extends Component {
    ...
    actions = {
                    ...
                    newInstrument.save().then(function(results){
                        ...
                        if(results.id != null) {
                            ...
                            for(var ctr = 0; ctr < anchorCount; ctr++) {
                                let gauge = parseInt(newInstrumentObject.gaugeLength);
                                let distance = parseInt(newInstrumentObject.headExclusionLength) + gauge;
                                if(!newInstrumentObject.stacked && ctr+1 > anchorCount / 2) {
                                    distance += (gauge + 20) / 2;
                                }
                                distance += (gauge + 20) * (ctr % (anchorCount / 2));
                                if (isNaN(distance)) {
                                    distance = 0;
                                }
                                store.createRecord('anchor', {number: ctr+1, distance: distance, instrument: results}).save();
                            }
                        }
        ...
        stackedChanged(object)
        {
            //console.log(document.getElementById('newInstrumentObjectBulbed').checked);
            this.set("stacked",document.getElementById('newInstrumentObjectStacked').checked);
        }
    }
}